export default /* glsl */`
	...
	vec3 fogBilinearCubeUV( sampler2D envMap, vec3 direction, float mipInt ) {
		...
		uv.y += 4.0 * ( exp2( FOG_CUBEUV_MAX_MIP ) - faceSize );

		uv.x *= FOG_CUBEUV_TEXEL_WIDTH;
		uv.y *= FOG_CUBEUV_TEXEL_HEIGHT;
	...
	vec4 fogTextureCubeUV( sampler2D envMap, vec3 sampleDir, float roughness ) {
		...
		float mip = clamp( fogroughnessToMip( roughness ), cubeUV_m0, FOG_CUBEUV_MAX_MIP );
